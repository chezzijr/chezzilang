...
fn genuine_deadlock_two_nurseries_deep_faults_instead_of_hanging() {
    let dir = std::env::temp_dir().join(format!("chz-ticket101-{}", std::process::id()));
    ...
    let Some(status) = status else {
        ...
        let _ = child.wait();
        panic!(
            "a receiver parked two nurseries deep on a channel with no possible sender must fault \
             `deadlock: …` at rc=1, not hang forever with no output (no exit within 10s)"
        );
    };
    ...
    assert!(
        ...
        "expected a `deadlock: …` fault, got: {stderr}"
    );
