...
const SLEEP_SYNCHRONISED_TESTS: [&str; 90] = [
    ...
    "recovered_nested_nursery_deadlock_is_fatal_at_one_worker",
    // TICKET-135 (W14-39): a deadline poll, not a happens-before edge.
    "a_sibling_fault_cancels_a_task_parked_in_its_nested_nursery_body_at_every_worker_count",
    ...
    // TICKET-134. Its 300ms `time.sleep_ms` is inside the fixture .chz program's SOURCE STRING. It
...
const CHZ_SLEEP_SITES: [(&str, &str); 8] = [
    ...
    // TICKET-147: the task being cancelled parks in this sleep; `started` orders the events.
    (
