...
| **W7-54** | `Type-system / Eq` | **A function value does not satisfy `Eq`, though `f == g` works — and `W7-41` propagated that wrong answer to a third spelling.** Measured on the post-change binary: `f := g` / `f == g` prints `true`; `f.eq(g)` is *type error: type fn(int) -> int has no method 'eq'*; `fn pick[T: Eq](a: T, b: T)` fed `g` is *type error: type fn(int) -> int does not satisfy Eq* — **both of those already failed identically on the pre-change binary**, so the operator-vs-method split predates this work. What `W7-41` added is the third: `struct Box[T]` with `fn eq(self, o: Self) -> bool where T: Eq` over a function payload printed `true` before and is now *cannot compare Box[fn(int) -> int] and Box[fn(int) -> int] for equality — Box[fn(int) -> int]'s `eq` requires fn(int) -> int: Eq*. **Rust — the owning ancestor for `Eq` — compiles the mirror**: `#[derive(PartialEq, Eq)] struct Boxy<T: Eq>(T)` with a `fn(i32) -> i32` payload builds clean and prints `true` (rustc 1.97.0, measured). So by the repo's north star this is drift, and the milestone widened its surface from 2 spellings to 3 | **NOT a 3-line carve-out, and carving it out at the operator would re-open exactly the operator-vs-method split `W7-41` exists to close.** `Ty::Func` classifies as `"?"` in `Checker::intrinsic_recv_kind` (`src/checker/proto.rs:285-307`), which matches no `INTRINSIC_PROTO_METHODS` row and therefore trips `grant_intrinsic`'s assert by design. Granting it needs the same four touch points Task 1 used for `option`/`result`: **(1)** a `Ty::Func` arm in `intrinsic_recv_kind`; **(2)** an `("Eq", "eq", "func")` row in `INTRINSIC_PROTO_METHODS` (`proto.rs:148`); **(3)** a `Recv` probe template in `intrinsic_grants_all_have_vm_arms` (`src/vm/tests.rs:16337`), which is what makes the grant a proof rather than a promise; **(4)** the VM dispatch. On (4), state it exactly: `do_method_call`'s object match (`src/vm/call.rs:1281`) has **no `Obj::Func`/`Obj::Closure` arm at all** — it has arms only for `Module`/`Struct`/`Enum`/`NewType` plus a terminal `_` (`:1565`) whose miss-only `intrinsic_proto_method` does carry a receiver-agnostic `("eq", 1)` arm (`:2541`). Whether that terminal arm already answers correctly for a function handle is **UNVERIFIED** — no well-typed program can reach it today, which is precisely what the probe in (3) would settle. Do not assume either way |
...
`W7-54` (a function value does not satisfy `Eq`, though `f == g` works — Rust compiles the mirror).
