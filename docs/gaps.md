...
is total, so a zero divisor is a legitimate value this widening exists to reach). Float `%`
(`BinOp::Mod`) is a separate, not-yet-started step — `gen_float` does not emit it.
