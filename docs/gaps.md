...
| **W7-57** | `:9538` | **`W7-47` residual ceilings: a run-wide `os.exit` is observed by POLLING waits only, so a party that is not polling outlives it.** Three measured shapes, all still divergent from Go: (a) a **CPU-spinning** party is never killed — no check at `jump_checked`'s back-edge (`src/vm/exec.rs:1482-1510`, the hottest path in the engine); (b) a fiber **snapshot-parked** on the netpoller or a channel cannot see a check at all — it needs a *wake*, which exists today only for a sibling inside the same nursery (scope cancel + `poller::drain_sched`), not for an unrelated eager job that cannot reach that nursery's `Arc<MnSched>`; (c) an **in-flight `sleep_ms` is uninterruptible** — measured `os.exit(3)` at 50 ms with a 300 ms sleeping nursery sibling exits with the right code but at **312 ms**, and a 500 ms sleeper still prints its line, where Go exits at 50 ms. A nursery-`spawn`ed `sleep_ms` has `native_reentry == 0` so it takes the timer-thread **offload** path, not `demote_block_sleep`; the `run_exit_err` rungs added to the three demote loops serve in-callback (`native_reentry > 0`) blocks only, and even there `thread::sleep` aborts the REST of the callback loop, not the sleep in flight | All three are "a wake, not a check". (a) is one relaxed load on the hottest path — add it only when a real program hits it. (b) is `mn_worker_loop`'s idle branch (`sched.rs:1054`, already a bounded `cv.wait_timeout`) checking the cell and tripping `terminate` + the existing `cancel_drain`/`drain_sched` abort branch. (c) wants the sleep to wait on a condvar the exit can notify, or to chunk at `DEMOTE_POLL_BACKOFF` like its demoted twin. None is a correctness hazard — the exit code is right in every case, only its latency is wrong — which is why they are filed rather than built. Note the direction: these DECLINE to act early, they never emit a confident wrong answer |
