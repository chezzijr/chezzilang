...
| **M24-3** | `Type-system / construction` | **Type-param shadowing is inconsistent — and M24 made it load-bearing.** A type parameter whose name equals a real type resolves to the **STRUCT** in static-call/ctor position but to the **PARAM** in annotation position: `struct T: q: int` + `fn f[T](x: T) -> T: return T(99)` → *expected return type T, found T*. **Pre-existing** — that repro needs no M24 feature and reproduces on the current binary with plain generics — but M24's lowering had to be ALIGNED to it (the compiler's witness arm sits last among the bare-receiver arms) so both halves agree on which `T` a `T.default()` means. **Go and Rust both let the type parameter shadow**, so the current precedence is the drift | Fixing the precedence is a resolution-order change that touches every bare-ident type position, not a witness change. Pinned meanwhile by `a_type_param_shadowing_a_type_name_resolves_the_same_in_both_halves` (`tests/chz/spec/static_witness_test.chz`), which asserts only that the two halves agree — an undefined answer is what M24 could not tolerate |
| **M24-4** | `Type-system / construction` | **The manifest-entrypoint witness rejection lives in `src/main.rs`.** `entrypoint = "src.main:main"` where `fn main[T: Default]()` is caught at check time by the `entry_fn` threaded through `main::type_check` → `checker::check_graph_with_entry`; a **library** caller invoking `chezzi::vm::run_file_with_entry` directly skips that thread and still sees the raw arity fault | CLI is covered (the only shipped consumer). Closing it means moving the gate below the library entry point, which changes `check_graph`'s signature for every caller |
