...
**Deferred: int↔float mixed arithmetic** (`1 + 2.0`), which needs a coercion model in the IR the
generator does not have today. Commented at `gen_float`'s definition so the next reader finds it
there, not only here.
...
`gen_emits_float_comparison` (added 2026-08-07 with comparison), and `gen_emits_float_div` (added
2026-08-07 with `Div`, mutation-proven red against `gen_float`'s pre-fix op list). Plus
`p0_shim_float_div_signed_zero`, a hand-written probe in the shape of `p0_shim_div_mod_negatives`
pinning `_chz_fdiv` against all four zero-divisor sign combinations plus `0.0/0.0` — the test that
fails if `_chz_fdiv`'s `copysign` pair is ever "simplified" back to a naive sign check. These are the
regression fence that matters: without them a refactor could revert to literal-only floats, int-only
calls, int-only comparisons, or a `Div`-less op list, and **every other test in the file would still
pass**, which is exactly the failure mode this row is about. All are red against the pre-fix
generator/shim. 5000-seed `difffuzz --floats` sweep: 0 findings.
