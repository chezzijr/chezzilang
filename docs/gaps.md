...
- **neither is `Unknown` nor contains one** (`contains_unknown_in_slot`, the helper already used a few
  lines above on this same path) — `Unknown` is the checker's don't-know value and would cascade, and
  `x := []` then `x := [1]` is a script-writer refinement, not a retype. Two *concrete* element types
  (`xs := [1]` / `xs := ["a"]`) are the same one-slot lie as any other and do reject;
...
#### W7-42r — three module-global retype shapes the `W7-42` rule does not reach — **OPEN**

The fix covers the single-name `let`/`:=` binding site whose previous binding is in `scopes[0]`. Three
shapes are still check-clean, all the same class:
...
All three were measured on the post-fix binary; (c) is identical on the pre-change binary too, so it is
not a regression. **(b) is the remaining half of the import case, not the whole of it:** the source-*earlier*-`import` shape (`import COUNT from lib` / a closure / `COUNT := "s"`)
...
- **(a) Docs, cheap, should not wait — LANDED 2026-08-10, commit `docs(stdlib): the CSPRNG surface
  ...
  on `crypto.secure_bytes`/`token_hex`.
