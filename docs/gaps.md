...
| **W7-52** | `Type-system / Eq` | **`Ty::Protocol` is the `Eq` bound guard's blind spot, in two places, and the first one CANNOT simply be refused.** (a) **The `Eq` walk fails open on it.** `Checker::eq_bounds_unsatisfied` fails CLOSED everywhere else — a table miss, an unresolvable shape or the in-progress budget all return `Some` (refuse), because the sole caller turns `None` into a `grant_intrinsic` PROMISE that the runtime can dispatch `eq`. `Ty::Protocol` is the one arm that answers `None` without a proof, and it is on the hot path, not in a corner: `Ty::result(inner)` is literally `Ty::Result(inner, Ty::Protocol("Error"))` (`src/checker/ty.rs:304-313`), so **every bare `Result[T]` carries an existential in its error slot** — refusing it un-grants `Result` wholesale and breaks the `("Eq", "eq", "result")` ratchet row. Measured consequence: `xs: List[Sized_] = [x]` / `xs.contains(y)`, where `Sized_`'s witness is `W7-41`'s `Box[Tag]`, is `ok: no type errors` then `runtime error (line 8, col 12): struct 'Tag' has no 'compare' method` on **both** engines. (b) **Protocol-existential operands leak past the `==` guard for a different reason.** `x: Sized_ = Box(Tag(1))` / `y: Sized_ = Box(Tag(2))` / `x == y` check-cleans and faults identically on both engines: `may_be_equal`'s `(Protocol(..), Protocol(..)) => true` arm (`src/checker/proto.rs:993`) short-circuits **before** the nominal arm, so a guard hooked there cannot see the operands' witnesses at all — structurally the same blindness as `W7-45`, one level up | Not closed by the `W7-41`/`W7-45` milestone, and neither half is a line edit. (a) needs the walk to carry a witness set rather than a `Ty` — i.e. the checker would have to know which concrete types can inhabit a protocol at this site, which it deliberately does not (existentials are erased). Refusing the arm is NOT an option: it is load-bearing for `Result`, per the code cited. (b) needs the question asked before the `(Protocol, Protocol)` short-circuit, which is the arm `docs/gaps.md` §B2 deliberately made permissive; narrowing it is that row's open question, not this one's. Both are **pre-existing in nature** — the same programs faulted the same way on the pre-change binary — so this row records a ceiling the milestone did not move, not a regression it introduced |
...
1.97.0 compiles the mirror, measured); and **`Ty::Protocol` cannot be refused** — see `W7-52`.

**Residuals, filed not closed:** `W7-52` (`Ty::Protocol`, both halves), `W7-53` (the erased call site),
and `W7-54` (a function value did not satisfy `Eq`, though `f == g` worked — Rust compiles the mirror),
**since FIXED 2026-08-12**.
...
**Residuals, filed not closed:** `W7-52` (protocol existentials) and `W7-53` — the erased call site,
whose third instance (`fn f[T: Hashable](xs: List[T]) -> int: return Set(xs).len()`) was found here.
