...
| ~~**M24-3**~~ | `Type-system / construction` | **FIXED 2026-08-10 — a type parameter now SHADOWS a same-named type in static-call position, in both halves.** `fn f[Item: Tagged](x: Item) -> int: return Item.tag()` beside a `struct Item` used to answer `7` (the struct); Rust answers `1` (`fn f<Item: D>(_x: Item) -> i32 { Item::tag() }` with `f(Counter)`, measured 2026-08-10) and so does Chezzi now. The checker's type-param arm moved ahead of its enum/struct static arms and the compiler's witness arm ahead of its own bare-receiver arms — both halves agree, on the ancestor's answer this time. The earlier fix had aligned them on the struct, which is what "correctness outranks agreement" is about. A shadowing param whose bound does NOT declare the method now says so (rustc: *no associated function `tag` found for type parameter*) instead of silently reaching the struct. Pinned by `a_type_param_shadows_a_type_name_of_the_same_name` (`tests/chz/spec/static_witness_test.chz`, RUNS) + `witness_type_param_shadows_a_same_named_type_in_static_call_position` | **Residual, unchanged:** the bare CTOR position still resolves to the struct — `struct T: q: int` + `fn f[T](x: T) -> T: return T(99)` → *expected return type T, found T*. That is `infer_named_call`'s bare-`Ident` callee, a different arm from the `Field` one M24 needed, and it needs no witness to reproduce. Under Rust's rule `T(99)` should be the param (and error: erased). Fixing it is a resolution-order change in the value-call path |
