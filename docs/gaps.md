...
| **M24-6** | `Diagnostics / spans` | **An interpolation fragment's reported COLUMN is an offset from the string literal's start, not the physical column, once a newline sits between them.** `s := """aaa\nbbb\ncc {10 / z} dd"""` faults at *line 2, col 59* where line 2 is 38 chars long (true position: line 4, col 8). Same for a `\n` ESCAPE before the fragment. The LINE is the literal's opening line by an older deliberate decision (`raw` is the post-escape payload, so a `\n` escape and a real source newline are indistinguishable — counting them would shift the line onto real, unrelated code); the COLUMN now keeps counting from the literal's start for the same reason, and because it must stay strictly increasing: it is the `WitnessTable` / `KeywordTable` key, and resetting it at a newline put two sibling fragments back on one key (a **silent wrong value**, measured 2026-08-10 and fixed by keeping the offset) | Deliberate, and the deliberate DIRECTION: an out-of-range column reads as degraded, where a shifted line accuses innocent code (`W7-12`: a wrong answer teaches distrust of every answer). Exact positions are not recoverable here — `raw` has already had its escapes processed and its delimiter stripped. The real fix is to carry a per-string source map (content-char index → source `Span`) on the `Str` token, which changes `Token::Str`, the AST node and every match site; it buys diagnostics only (the keys are already correct), so it is its own task. **Cross-ref: `W7-49` is the general statement of the constraint this row's column rule serves** — a `Span` in this checker IS a cross-half table key, which is why the column must keep counting here and why `Span` grew a `file` id there. Both are the same invariant (a key must be injective) read in two axes: within one lexed source, and across modules |
