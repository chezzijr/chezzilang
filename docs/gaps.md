...
| ~~**W7-52**~~ | `:10243` | **RESOLVED 2026-08-12 — not a bug: this is Go's own answer to comparing protocol-typed (interface-typed) values, measured, not assumed.** Protocols are Go interfaces (`CLAUDE.md`), and Go compiles interface `==` unconditionally, panicking at the point it actually runs when the dynamic type turns out uncomparable — measured, Go 1.26: `panic: runtime error: comparing uncomparable type main.Tag`. Chezzi does the identical thing at the identical point, both engines: `x: Sized_ = Box(Tag(1))` / `y: Sized_ = Box(Tag(2))` / `x == y` check-cleans then `runtime error (line 12, col 16): struct 'Tag' has no method 'compare'`; `xs.contains(y)` over `xs: List[Sized_]` faults identically. Both reproduce byte-identical on the pre-change binary — nothing regressed, nothing was fixed, because there was nothing to fix | Neither arm can be narrowed even if narrowing were wanted. (a) `eq_bounds_unsatisfied_rec`'s `Ty::Protocol` catch-all (`src/checker/proto.rs:2810`) cannot be turned into a refusal: `Ty::result(inner)` is literally `Ty::Result(inner, Ty::Protocol("Error"))` (`src/checker/ty.rs:304-313`, re-confirmed), so every bare `Result[T]` carries an existential in its error slot — refusing the arm un-grants `Result` wholesale and breaks the `("Eq", "eq", "result")` ratchet row. (b) `may_be_equal`'s `(Protocol(..), Protocol(..)) => true` arm (`src/checker/proto.rs:997`) must stay ahead of the nominal arm — it short-circuits before either operand's witness is visible, and moving or narrowing it wrong-rejects a legitimate protocol pair; that arm's permissiveness is §B2's own open question, not this row's. The checker deliberately erases existentials — it does not know which concrete types inhabit a protocol at a given site. Refusing every protocol-typed `==` would reject working programs; guessing would emit a confident wrong answer. Per this file's own rule, an unsure verdict must decline, which is exactly what deferring to a clean runtime fault does. Locked by `tests/chz/spec/eq_protocol_existential_test.chz` (both engines) |
...
**Residuals:** `W7-52` (`Ty::Protocol`, both halves) — **since RESOLVED 2026-08-12, not a bug** — and
...
**Residuals:** `W7-52` (protocol existentials) — **since RESOLVED 2026-08-12, not a bug** — and
...
### W7-52 — comparing a protocol-typed value defers to the witness at runtime — **RESOLVED 2026-08-12, NOT A BUG**
...
withdrawn.** Protocols in Chezzi are Go interfaces (`CLAUDE.md`: *"Go for concurrency/interfaces"*),
and Go's answer to comparing two interface-typed values is to compile the comparison unconditionally
and panic at the point it actually runs, if the dynamic type turns out uncomparable. Chezzi gives the
same answer at the same point, for the same reason.
...
`slices.Contains` over `[]Sized` panics the same way, for the same reason.

**Both programs were re-run against the pre-change binary
(`/tmp/.../scratchpad/chezzi-prehead`) and produce byte-identical output on all four combinations
(HEAD × prehead, M:N × `--serial`).** Nothing regressed and nothing was fixed, because there was
nothing to fix — the row simply had the wrong verdict.

**Why neither arm can be narrowed, even if narrowing were wanted:**

1. **`eq_bounds_unsatisfied_rec`'s `Ty::Protocol` catch-all (`src/checker/proto.rs:2810`) cannot
   become a refusal.** `Ty::result(inner)` is literally `Ty::Result(inner, Ty::Protocol("Error"))`
   (`src/checker/ty.rs:304-313`, re-confirmed for this row) — every bare `Result[T]` carries an
   existential in its error slot. Refusing the arm un-grants `Result` wholesale and breaks the
   `("Eq", "eq", "result")` ratchet row.
2. **`may_be_equal`'s `(Protocol(..), Protocol(..)) => true` arm (`src/checker/proto.rs:997`) must
   stay ahead of the nominal arm.** It short-circuits before the arm that could see the operands'
   witnesses; moving or narrowing it wrong-rejects a legitimate protocol pair (one concrete type can
   inhabit two unrelated protocols at once). That arm's permissiveness is this file's §B2 open
   question, not this row's.

The deeper reason is the same one `W7-41`/`W7-45`/`W7-53` all lean on: the checker deliberately
erases existentials, so it does not know which concrete types inhabit a protocol at a given call
site. A checker that refused every protocol-typed `==` would reject working programs (`Box[int]`
compares fine — see the positive control below); one that guessed would emit a confident wrong
answer. This file's own rule for an unsure verdict is to **decline**, never to guess — deferring to a
clean, catchable runtime fault is exactly that.

**Tests.** `tests/chz/spec/eq_protocol_existential_test.chz` (4 tests, gated serial==M:N by
`test_runner::chz_suite_passes_both_engines`): the operator and `.contains()` faults above, each
asserting the recovered message names the failing method (`compare`); and the positive control on
the same `Box[T]` shape with a `Comparable`-satisfying payload (`Box[int]`), proving a protocol-typed
`==`/`.contains()` gives the right answer and does not fault when the witness genuinely satisfies the
bound. All four were run against the pre-change binary first and pass there identically, as expected
for a verdict-not-fix task — the test pins existing behavior against regression, it does not prove a
fix.
