...
`concurrency.RwShared`/`Shared` over such a container. Separately, the three routes that could **obtain**
a bad `Set`/`Map` without touching a literal or a ctor — annotation, fn return type, struct field type —
all reject, so the offending TYPE becomes **unspellable** and every `Map`/`Set`/`RwShared` method is
unreachable *by construction* rather than gated one by one.
