...
|---|---|---|---|
| **W7-56** | `:9538` | **A live eager `Executor` job does not veto the nursery deadlock predicate, so a `parallel:` whose child waits on that job is killed by a FALSE `deadlock` — a working program, declared broken.** Measured on the **unmodified pre-`W7-47` binary** (HEAD `a9989dc0`, rebuilt from a clean stash), no `os.exit` anywhere: a top-level `ch := Channel[int]()`, an `ex.submit(feeder)` whose `feeder` sleeps 50 ms then `ch.send(7)`, and `parallel: spawn waiter()` where `waiter` does `ch.recv()` → `child waiting`, then `runtime error: deadlock: every task in this parallel: block is blocked on a channel it cannot proceed on … the nursery cannot progress`, **rc=1 at ~11 ms — the feeder never even ran**. Go prints `job sending` / `child got 7`. **Pre-existing; NOT introduced by `W7-47`** (found while investigating a `W7-47` review charge, which is why the exit variant of the same shape could not be fixed by any exit-observation check: the verdict fires 39 ms before the exit exists) | `MnSched::is_deadlocked` counts only the nursery's OWN fibers and has no veto for outstanding `ExecRegistry` jobs — the same veto `inflight` already provides for a sleeping fiber, and the same one `awaiting_builder` provides for an inline builder. Textbook **`parked-is-not-stuck`**: the predicate is built from what looks idle rather than from what is IMPOSSIBLE, and an outstanding job that can still `send` makes the wait satisfiable. Deliberately NOT fixed inside `W7-47` — widening this predicate has `W7-12`'s history (three predicates in a row faulted healthy programs, all fully green), so it needs its own task with the Go-paired programs. The two-line repro above is ready to paste |
