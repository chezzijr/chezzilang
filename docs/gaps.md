...
**Root cause.** `Ty::Func` had no arm in `Checker::intrinsic_recv_kind` (`src/checker/proto.rs:285`),
...
1. `Ty::Func { .. } => "func"` added to `Checker::intrinsic_recv_kind` (`src/checker/proto.rs:285`).
...
**Tests.** `tests/chz/spec/eq_func_test.chz` (6 tests, gated serial==M:N by
...
itself; and the `W7-41`-regressed `Box[fn]` spelling restored to `true`. Plus
`checker::tests::function_value_satisfies_eq_by_identity` (the `[T: Eq]`-fed-a-function case) and the
known-gap test at `checker::tests::eq_operator_enforces_the_receivers_eq_where_bounds` updated from
`entry_rejects` to `entry_ok` for the function-payload `Box[T]` row (it now pins the fix, not the bug;
`Box[Channel]` stays rejected, per rustc). Every new/changed assertion was run against the pre-change
binary first and measured failing with the exact messages quoted above.
