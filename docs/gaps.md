...
The twin oracle one directory over, **`panicfuzz::classify`** (`src/panicfuzz/run.rs:96-101`),
...
`py_ok` false and falls through to the ordinary PythonFault/BothError arms, which is the right
non-finding shape for "the reference interpreter crashed on our generated input."
