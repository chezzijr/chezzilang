...
|---|---|---|---|
| **W7-58** | `:9538` | **A genuine nursery deadlock whose outstanding eager job is ALSO blocked forever now HANGS instead of faulting.** Measured: a job doing `other.recv()` on a channel nobody feeds, beside `parallel: spawn waiter()` doing `ch.recv()` on another — pre-`W7-56` this faulted `deadlock` at 7 ms; now it hangs (25 s timeout, 3/3). Introduced deliberately by `W7-56`'s coarse `outstanding_jobs > 0` veto, which says *a job exists*, not *a job can feed you*. The old fault was correct **by accident**, from the same unsound counting that made `W7-56` fault live programs | **Silent, not wrong — the direction `parked-is-not-stuck` prescribes** (correct > silent > wrong), which is why it shipped. The reason no second detector rescues it: neither a parked fiber nor a `main` sitting inside a nursery join is a counted party (`is_counted_party` = `owns_os_thread() && native_reentry == 0`, and `owns_os_thread` is false by construction inside a nursery), so `quiesce::quiesced` never runs. Two candidate fixes, both bigger than `W7-56`: narrow the veto to *jobs that can still run* (needs a job↔party identity the registry deliberately does not carry — `quiesce.rs:25-28` refused exactly that counter, and a naive `outstanding > parties.len()` mis-fires because `parties` also counts `main` and degenerates when the nursery is opened BY a job), or make a nursery-joining `main` a counted party (a rework of the counting model: `live = 1 + Σ outstanding` counts only threads with no scheduler under them, and a `PartyWait` for the join would need `satisfiable() == !is_deadlocked()`, which is circular). **Do not attempt a fourth precise predicate without the identity** — three have already shipped green and faulted healthy programs (`W7-12`) |
