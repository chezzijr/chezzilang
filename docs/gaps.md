...
`ZeroDivisionError`) and inexact quotients widen the formatting surface a lot at once — it wants its
own pass with a non-zero-divisor discipline like `gen_int`'s. (2) Int↔float mixed arithmetic
...
`gen_emits_float_binop`, `gen_emits_non_int_call`, `gen_emits_non_int_index`, and (added with float
comparison, 2026-08-07) `gen_emits_float_comparison`. These are the regression fence that matters:
without them a refactor could revert to literal-only floats, int-only calls, or int-only comparisons
and **every other test in the file would still pass**, which is exactly the failure mode this row is
about. All are red against the pre-fix generator.
