...
- **Shipped (M21):** nominal **`newtype`** — `newtype Name = <type>` (optionally with a method block)
  ...
  `Eq` from no newtype at all. A numeric newtype *declaring* such a method is a decl-site error. **Generic newtypes** (`newtype Stack[T] = List[T]`) are methods-only
