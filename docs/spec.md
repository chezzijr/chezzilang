...
unbounded `Channel[T]()`, rendezvous `Channel[T](0)`, or bounded `Channel[T](cap)` with backpressure,
...
cleanly with a clear error: a suspension **with a pending `defer`** (`defer` is
...
**`Iterable[T]` protocol + `.iter()`** (additive over the `Iterator[T]` iteration model).
`Iterable[T]` promises `.iter() -> Iterator[T]` (a fresh COMPOSABLE cursor);
...
- **Std modules v1 (shipped, M6c):** `std.math`/`std.io`/`std.os` (native-Rust via the FFI seam),
  ...
  negation, `MIN / -1`, `math.abs(MIN)`, and integer `List.sum()` (checked add) —
  is a *recoverable
  panic* (`"integer overflow in <op>"`, catchable by `recover:`), never a silent wrap and never a host
  crash. `%` never overflows: `MIN % -1` is representable (`0`, matching Go and Python) since a true
  mathematical remainder always fits in the dividend's own range — only `/` can overflow at that input
  (the true quotient `2^63` doesn't fit an `i64`). **Float arithmetic is total IEEE-754** (the policy
  diverges by type): a `float` op *never*
  faults — `1.0/0.0` is `inf`, `-1.0/0.0` is `-inf`, `0.0/0.0` and `5.0%0.0` are `NaN`, and
  `math.sqrt(-1.0)` is `NaN`. **Float `%` is `fmod`: the sign follows the DIVIDEND** (`-7.0 % 3.0`
  is `-1.0`, `7.0 % -3.0` is `1.0`), the same rule Chezzi's integer `%` already uses and matching
  C/Rust's `%`/Go's `math.Mod` — a deliberate divergence from Python's floored `%`, which follows
  the divisor instead (`-7.0 % 3.0` is `2.0`) and raises `ZeroDivisionError` on a zero divisor,
  where Chezzi's is total (`5.0 % 0.0` is `NaN`). `inf`/`NaN` are ordinary values; inspect them with `math.is_nan` /
...
- **Shipped:** **enum methods** — `fn name(self, …)` blocks after an enum's variants, mirroring struct
  ...
  satisfaction so an enum can define `str`/`hash`/`add`/`compare` for `Stringable`/`Hashable`/
...
>   carried by `vm::Obj` (`Native`).
> - ✅ **`Host` trait** (`src/native/mod.rs`) — the context a native fn uses
>   (`arg_int`/`arg_float`/`arg_str`, stdout/stderr/stdin, args/env/cwd) so a binding is written
>   once and runs on the VM (heap handles). Returns flow back as a
>   `NativeRet`, lowered to a `Value` *after* the call (GC-safe).
...
>   the SAME `Host`/`NativeRet` seam as any other native fn.
...
>     cannot guard state it does not own). Use only thread-safe/reentrant C entry points — there is no
>     sequential engine to fall back to.
...
supported shapes, and the three walls that remain (a type parameter of the enclosing TYPE, a
witness-taking fn read as a function VALUE, an undetermined `T`; the `spawn`/`defer` statement target
