...
4c-net, `native_module_sig` retains only **concurrency** (opcode type-license) + **ffi** (extern).
...
**Roadmap:** after 4b/4f/4c-net, `native_module_sig` retains only **concurrency** (opcode type-license)
and **ffi** (extern) — the remaining phase-4c targets (opcode/extern binding). `grammar.bnf` unchanged
(native-decl grammar + param defaults exist from 3a/4a; conformance green).
...
`native_module_sig` content after 4d/4e/4f/4c-net:** only `ffi` (extern) and `concurrency` (opcode
type-licensing) — the remaining phase-4c targets (extern/opcode binding). (`net` migrated in 4c-net.)
