...
`native_module_sig` content:** `encoding`/`crypto`/`uuid`/`time` (pure-function, a parallel batch);
`request`/`process` (fields-only synthetic-struct types `Response`/`ProcResult`, another batch);
`net`/`ffi`/`concurrency` (opcode/methoded/type-licensing — deferred to focused passes).
