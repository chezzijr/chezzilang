...
- ✅ **Two check-OK → runtime-fault holes closed: a bound method is no longer readable as a value, and `index`/`set_index` must now be V-coherent** (2026-07-13, `auto-task/checker-method-value-index-coherence`) — both were checker-only accepts of programs the runtime cannot execute; both fixes are checker-only rejects (no compiler/VM change → parity- and perf-neutral). **(1) Bound-method value.** `g := s.get` on a `struct S` with `fn get(self) -> int` type-checked, then faulted on both engines with `no field 'get' on S(n=5)`. Root cause: `infer_field`'s `Ty::Struct` arm (`src/checker/pattern.rs`) fell back to the METHOD table after the data-field lookup missed and handed back a `Ty::Func` still carrying the **un-bound `self` slot typed `Ty::Unknown`** — nothing lowers that (the compiler emits a plain field load), and the `?` self slot **laundered types**: `g("anything")`, `apply(s.get, s)` against a `fn(S) -> int` param, `xs := [s.get, s.get]`, and `g := self.get` inside a method body ALL checked OK and then faulted. The four sibling receiver kinds (enum / newtype / protocol existential / the assign path) already rejected correctly, which pinned the seam exactly. Methods are **not first-class values** (nowhere sanctioned in the docs — an accidental accept, not a feature): the fallback is deleted, so control falls through to the sibling error, with a method-aware hint — `type S has no field 'get' ('get' is a method — methods are not values: call it (\`x.get(…)\`) or wrap it (\`fn(): x.get()\`))`. The misleading follow-on arity message (`'closure' expects 1 argument(s), got 0`) disappears for free: the expr is now `Ty::Unknown` and `infer_call`'s Unknown arm stays silent — exactly ONE diagnostic (asserted). Fields are searched BEFORE methods in the same arm, so the closest neighbour — a genuinely **fn-TYPED field** (`f: fn(int) -> int`, `s.f(3)` AND `g := s.f; g(3)`) — still works, as do method calls, chained calls, calls on a nested field, a field whose name collides with a method, and static/associated CALLS (a bare `S.make` VALUE is not legal today anyway — `unknown name 'S'`). **(2) `index`/`set_index` V-incoherence.** A struct with `index(self, int) -> str` but `set_index(self, int, val: int)` accepted `s[0] += 1`, then faulted with `cannot apply Add to str and int`. `x OP= v` is defined as exactly `x = x OP v` (`docs/syntax.md` §3), so the compound's LHS must be typed from **`index`'s RETURN**; the direct index-assign path took it from `set_index`'s `val` instead and checked `set_index` **in isolation**, never enforcing the `IndexSet[K, V]` coherence the **bound** path (`[T: IndexSet[int, str]]`) already enforced — the operator path and the protocol path disagreed, and the operator path was the wrong one. Fix at the root: `index_set_kv` (`src/checker/sig.rs`, one caller) now resolves the read side through the existing `index_kv`, requires `K` **and** `V` agreement, and returns the **index-derived** pair — matching the compiler's `Dup2 → GetIndex → op → SetIndex` lowering. The comparison happens AFTER the struct param substitution (so a generic `Buf[T]` with `index -> T` / `set_index(_, T)` survives every instantiation) and an `Unknown` slot is treated leniently (a partially-inferred receiver is not a disagreement). An incoherent pair now errors at check time on `s[k] = v` **and** `s[k] OP= v` alike, with protocol-matching wording via a new `index_assign_error` sibling — `type S does not satisfy IndexSet (index returns str but set_index's val is int)` — while the missing-method cases keep the existing vague `cannot index-assign into RO`/`WO` verbatim. Behaviour preserved: coherent user `Index`/`IndexSet` pairs (read / write / compound / negative index / all open+step slice bounds), `index`-only (read-only) and `set_index`-only structs, builtin targets (`m[k] += 1`, `xs[i] += 1`, `obj.f += 1`), and the once-only evaluation of a side-effecting index expr (Python parity). **Blast radius: ZERO in-repo `.chz`** — no example/std/bench/test reads a bound method or defines an incoherent pair (`examples/slicing.chz`'s `Ring` and the parity `BUF_PROG` are coherent); the full golden suite over `examples/*.chz` is byte-identical. +10 checker tests RED-first via `check_entry`/`entry_ok`/`entry_rejects` (the real `build_graph` + `check_graph` path) incl. every launder variant and the no-over-rejection guards, + 3 two-engine RUN guards in `src/vm/parity_tests.rs` (coherent `str`-valued IndexSet, index-expr-evaluated-exactly-once, fn-typed-field-as-value). Docs: `docs/syntax.md` (§3 compound-assign ≡ `x = x OP v` + the V-coherence rule + once-only index eval; §7 methods are not first-class values — call or wrap; §7b `IndexSet` requires a coherent `Index`).
