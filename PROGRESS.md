...
4a** = `native struct` syntax + the companion-stub loader, with `regex.Match` migrated (fields-only) —
**DONE** (see the phase-4a entry above). **Phase 4b** = bodyless native **method**-sig→native binding +
migrate the remaining Tier-2 native (Rust-backed) TYPES (`Shared`/`RwShared`/`Atomic`/`Executor`,
