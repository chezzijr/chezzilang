...
> overridden only in `VmHost`; no `std::env::set_var` third source). `chdir` mutates the REAL process cwd
> (like `getcwd` reads it) — **process-global**, shared by all M:N workers (ponytail ceiling, same as
> Python/Go). Queries are engine-agnostic (serial==M:N). Tests: `golden_os_setenv_environ_consistency`
> (proves setenv↔env↔environ), `golden_os_queries` (shape + engine agreement), `golden_os_chdir` (Ok/Err
> under `FS_SCRATCH_LOCK` + cwd-restore). Example `examples/os_info.chz` (no `.expected`). Docs:
> `docs/stdlib.md §std.os`, `docs/gaps.md §6` (SHIPPED; the os.env/process.cmd note reworded — os.env axis
> resolved, child-process axis unchanged by design). Deferred: `os_name` alias, Windows `USERPROFILE`,
> signals/atexit, metadata-reader.
>
