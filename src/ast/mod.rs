...
// (The widen-SUPPRESSION verdict for a `List[Any]` slot is NOT a hint on this channel: it is derived
// from the expected slot type at the literal itself, so it holds at every position a slot reaches a
// literal, not just an annotated `let`. See `crate::checker::any_elem_slot` / `ListWidenTable`.)
