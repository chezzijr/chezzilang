...
pub enum Op {
    ...
    /// The spec was parsed at compile time; type/value mismatches surface as runtime errors. A
    /// nested width/precision field (`dyn_width`/`dyn_precision`) is popped first: the operands are
    /// value, width, precision (top).
